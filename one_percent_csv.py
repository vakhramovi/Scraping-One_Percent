...
# 	all_elements = []
# 	with open("one_percent.csv", "w", encoding="utf-8") as file_csv:
# 		writer
...
def get_data():
    ...
    with open("one_percent.csv", "w", newline="", encoding="utf-8") as file_csv:
        writer = csv.DictWriter(file_csv, fieldnames=["Name", "Type", "Website", "Currency", "Summary", "Country", "Linkedin", "Instagram", "Facebook", "Youtube", "Twitter"])
        writer.writeheader()
        for page in range(1, pages + 1):
            response = s.get(f"https://e1k3unhdf2.execute-api.us-east-1.amazonaws.com/search?accountType=business&location=usa&pageSize=20&page={page}").json()
